...
mod tests {
    ...
    fn inline_component() -> ProvidesTrimMaterial {
        ProvidesTrimMaterial::new(RegistryHolder::direct(TrimMaterialValue::new(
            ...
            TextComponent::plain("Custom trim material"),
        )))
    }
    ...
    #[test]
    fn inline_material_round_trips_and_hashes_its_direct_record() {
        init_vanilla_registry();
        ...
        assert_eq!(
            compound
                .string("palette_id")
                .map(|palette_id| palette_id.to_str().into_owned()),
            Some("minecraft:custom".to_owned())
        );
    }
