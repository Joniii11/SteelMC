...
use steel_utils::{
    ...
    random::{Random as _, RandomSource, generate_unique_seed, legacy_random::LegacyRandom},
};
...
/// Vanilla synchronized `Level.getRandom()` source.
struct WorldRandom {
...
pub struct World {
    ...
    /// Vanilla persistent named random sources.
...
impl World {
    ...
    pub(crate) async fn new_with_config_and_encoding_pool(
        ...
        let random_sequences = Arc::new(RandomSequences::load(saved_data.clone(), seed).await?);
        let mut level_data =
    ...
    pub async fn cleanup(&self, total_saved: &mut usize) {
        ...
        if let Err(error) = self.random_sequences.save().await {
            log::error!("Failed to save world random sequences: {error}");
        }
    ...
    /// Uses a named vanilla random sequence.
    ...
    /// Runs a callback against Vanilla's synchronized `Level.getRandom()` source.
