...
fn sound_holder_is_within_range(
    sound: &SoundEventHolder,
    volume: f32,
    distance_squared: f64,
) -> bool {
    let range = match sound {
        SoundEventHolder::Registry(sound) => sound.range(volume),
        SoundEventHolder::Direct { fixed_range, .. } => {
...
impl World {
    ...
    /// Plays a holder-backed sound at an exact world position.
    pub fn play_sound_holder_at(
        ...
        let Ok(encoded) =
            ...
            log::warn!("Failed to encode holder sound packet");
            return;
        };
        ...
        for entity_id in self.player_area_map.get_tracking_players(chunk) {
            ...
            let dy = player_pos.y - pos.y;
            let dz = player_pos.z - pos.z;
            if sound_holder_is_within_range(&sound, volume, dx * dx + dy * dy + dz * dz) {
                player.connection.send_encoded(encoded.clone());
            }
        }
    }
    ...
    /// Plays a holder-backed block sound at the center of a block position.
