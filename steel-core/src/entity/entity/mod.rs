...
impl CollisionRestitutionInput {
    fn new(
        ...
        Self {
            ...
            vertical_collision_below: entity.vertical_collision_below(),
    ...
    fn compute(self) -> CollisionRestitutionResult {
        ...
        let mut velocity = self.current_velocity;

        if self.x_collision {
            velocity.x = -self.current_velocity.x * restitution;
        }
        if self.z_collision {
            velocity.z = -self.current_velocity.z * restitution;
        }
        ...
        CollisionRestitutionResult { velocity, bounced }
    }
}
...
pub trait Entity: EntityEventSource + ErasedType + Send + Sync + 'static {
    ...
    /// Marks the entity for vanilla `Entity.syncPosition` handling.
    ...
    /// Clears the movement-sync marker after `ServerEntity.sendChanges` handles it.
    ...
    /// Returns vanilla `Entity.getAirDrag()`.
    ...
    /// Returns whether this entity uses omnidirectional air movement.
    fn omnidirectional_air_mover(&self) -> bool {
        false
    }

    /// Returns vanilla `Entity.getEntityBounciness()`.
    ...
    /// Returns vanilla block bounce restitution, including non-living scaling.
    ...
    fn move_entity(&self, mover_type: MoverType, delta: DVec3) -> Option<MoveResult> {
        ...
        if self.can_simulate_movement()
            && ((movement.y.abs() > 0.0 && result.vertical_collision)
                || result.horizontal_collision)
    ...
    /// Mirrors vanilla `Entity.restituteMovementAfterCollisions`.
    fn restitute_movement_after_collisions(
        ...
        let restitution = CollisionRestitutionInput::new(
            self.as_entity_event_source(),
            result,
            movement,
            effect_state,
