...
impl FeatureDecorationRunner {
    ...
    fn create_poplar_tree_foliage(
        ...
        for y in (1..foliage_height - 3).rev() {
            Self::place_poplar_tree_leaves_row(
                region,
                registry,
                random,
    ...
    fn replace_poplar_leaves_with_logs(
        ...
        for dx in -current_radius..=current_radius + offset {
            for dz in -current_radius..=current_radius + offset {
                ...
                let corner_blocks_to_cut = Self::poplar_corner_blocks_to_cut(
                    dx,
                    dz,
                    current_radius,
                    Self::poplar_row_is_partial(foliage_height, y),
                    flip_rhombus_shape,
                );
                if !Self::poplar_is_within_rhombus(
                    current_radius,
                    abs_dx,
                    abs_dz,
                    corner_blocks_to_cut,
                    2,
                ) || !(abs_dz == 0 && current_radius - abs_dx >= 4
                ...
                let trunk_state = Self::with_axis_if_present(trunk_state, axis);
                placement.set_foliage(region, pos, trunk_state);
            }
        }
    }
    ...
    fn poplar_foliage_should_skip_location(
        ...
        let partial_rhombus = Self::poplar_row_is_partial(foliage_height, y);
        let corner_blocks_to_cut = Self::poplar_corner_blocks_to_cut(
            dx,
            dz,
            current_radius,
        ...
        let abs_dx = abs_i32(dx);
        let abs_dz = abs_i32(dz);
        if partial_rhombus && (abs_dx == current_radius || abs_dz == current_radius) {
            return true;
        }

        let additional_side_removal = i32::from(random.next_f32() <= side_hole_chance);
    ...
    const fn poplar_row_is_partial(foliage_height: i32, y: i32) -> bool {
