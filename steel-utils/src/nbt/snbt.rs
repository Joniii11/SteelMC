...
fn parse_float_token(token: &str) -> Result<NbtTag, String> {
    ...
    match kind {
        FloatKind::Float => {
            ...
            Ok(NbtTag::Float(if value == 0.0 { 0.0 } else { value }))
        }
        FloatKind::Double => Ok(NbtTag::Double(if value == 0.0 { 0.0 } else { value })),
    }
}
...
mod tests {
    ...
    #[test]
    fn parses_negative_zero_floats_like_vanilla_nbt_ops() {
        let float = parse_snbt("-0.0f").expect("float literal parses");
        let NbtTag::Float(float) = float else {
            panic!("expected float tag");
        };
        assert_eq!(float.to_bits(), 0.0_f32.to_bits());

        let double = parse_snbt("-0.0d").expect("double literal parses");
